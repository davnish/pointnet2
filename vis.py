...
def visualize_model(model_name):
    ...
    tiles = np.load(os.path.join("data_tald", 'test', 'not_norm', "not_norm_25_4096.npz"))
    data = tiles['x'].reshape(-1, 3)
    label = tiles['y'].reshape(-1)
    model = PointnetSeg().to('cuda')

    model.load_state_dict(torch.load(os.path.join(f"{model_name}.pt")))
